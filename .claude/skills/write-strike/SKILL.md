...
1. **Brand voice & editorial guidelines:** `.claude/skills/bolt-TOV-and-guidelines/SKILL.md`
...
2. **Person-specific voice profiles:** `.claude/skills/bolter-tones/SKILL.md`
...
3. **Buyer personas:** `.claude/skills/bolt-buyer-personas/SKILL.md`
...
The bolt-TOV-and-guidelines, bolter-tones, and bolt-buyer-personas source skills evolve over time, and the GEO drafting layer (`.claude/skills/write-strike/references/SEO-GEO-drafting.md`) moves faster than all of them. Once a month, ask the user using AskUserQuestion:
...
- **TOV & guidelines changed**: Re-read `.claude/skills/bolt-TOV-and-guidelines/SKILL.md` and its references. Summarize what changed, then ask: "Should I update write-strike to reflect these changes?"
- **Bolter Tones changed**: Re-read `.claude/skills/bolter-tones/SKILL.md` and any updated tone profiles. Summarize what changed, then ask: "Should I update write-strike to reflect these changes?"
- **Buyer Personas changed**: Re-read `.claude/skills/bolt-buyer-personas/SKILL.md`. Summarize what changed, then ask: "Should I update write-strike to reflect these changes?"
- **SEO-GEO drafting refresh**: The fastest-moving layer; review it every month regardless. Re-read `.claude/skills/write-strike/references/SEO-GEO-drafting.md` and scrutinize the items tagged FAST-MOVING (per-engine citation behavior, the exact word and passage numbers, llms.txt and markdown conventions) against current reality. The DURABLE spine rarely changes. The SEO tooling in `bolt-seo-geo` runs on a slower quarterly review, not this monthly one.
...
| Customer story | "customer story", "case study" | Template | `.claude/skills/bolt-content-formats/references/customer-story-template.md` |
| Social | "social", "LinkedIn", "X post", "tweet", "Reddit" | Light | None |
| Email | "email", "newsletter", "drip", "email sequence" | Light | None |
| Website copy | "website copy", "landing page", "hero copy" | Medium | `.claude/skills/write-strike/references/web-copy-best-practices.md` + `.claude/skills/write-strike/references/SEO-GEO-drafting.md` |
| Long-form | "whitepaper", "ebook", "guide", "manual", "survey report", "research report", "findings" | Full | `.claude/skills/write-strike/references/SEO-GEO-drafting.md` |
| Ad copy | "ad", "ad copy", "campaign copy" | Medium | `.claude/skills/write-strike/references/schwartz-copywriting.md` + `.claude/skills/write-strike/references/schwartz-5x5-matrix.md` |
| Executive brief / 1-pager | "exec summary", "executive summary", "1-pager", "one-pager" | Medium | `.claude/skills/bolt-content-formats/references/minto-scaffold.md` |
| Byline | "byline", "thought leadership", "op-ed", "ghostwrite" | Full | `.claude/skills/bolt-content-formats/references/minto-scaffold.md` |
| Webinar | "webinar", "webinar BOM", "webinar content" | Template | `.claude/skills/bolt-content-formats/references/webinar-bom-template.md` |
| Sales enablement | "sales deck copy", "battle card", "objection handling" | Template | `.claude/skills/bolt-content-formats/references/sales-enablement-template.md` |
| Creator brief | "creator brief", "influencer brief" | Template | `.claude/skills/bolt-content-formats/references/creator-brief-template.md` |
...
Once the user selects an audience, read their persona from bolt-buyer-personas (`.claude/skills/bolt-buyer-personas/SKILL.md`) and use it to shape every decision.
...
**Customer story:**
- Load `.claude/skills/bolt-content-formats/references/customer-story-template.md` and work through its Pre-Draft Checklist
...
**Webinar:**
- Load `.claude/skills/bolt-content-formats/references/webinar-bom-template.md` and work through the Event Details section first
...
**Sales enablement:**
- Load `.claude/skills/bolt-content-formats/references/sales-enablement-template.md` and work through the relevant asset's checklist
- Asset type (battle card, objection handler, competitive one-sheet)
- Target buyer persona
- Key objections or competitive positioning

**Creator brief:**
- Load `.claude/skills/bolt-content-formats/references/creator-brief-template.md` and work through its Pre-Brief Checklist
...
Before the Schwartz diagnosis, for content where the customer's own words matter (customer stories, conversion web copy, sales enablement, nurture email), do a quick VoC pull using `.claude/skills/write-strike/references/voc-message-mining.md`: mine reviews, Reddit, support, and call transcripts into the six buckets, then let them supply real language and surface objections. This is a sharpening tool, not a gate. For search-led or creative work (SEO blogs, ad copy, social), keep it as an opportunistic gut-check, never a blocker. The running question is "is this validated by the customer, or am I asserting it?", aimed at claims about the customer's pain or experience, not every line.

### Schwartz messaging diagnosis (blog, website copy, ad copy, landing pages, sales enablement)

Before drafting persuasion-heavy content, diagnose the reader's position using the Schwartz framework. Read `.claude/skills/write-strike/references/schwartz-copywriting.md` for the full model. Answer two questions:
...
Then consult `.claude/skills/write-strike/references/schwartz-5x5-matrix.md` to find the intersection. The matrix tells you the headline strategy, lead approach, copy length, proof type, and CTA style for that specific combination.
...
**Customer story outline follows** the template structure in `.claude/skills/bolt-content-formats/references/customer-story-template.md`: headline, snapshot, setup, problem, turn, build, results, close, CTA.

**Executive briefs / 1-pagers, bylines, and sales arguments** follow the answer-first document scaffold in `.claude/skills/bolt-content-formats/references/minto-scaffold.md`: governing thesis up top, an SCQA lead, then three to five MECE supporting points.
...
For social, email, ads, and other short-form quick hits (the Light-workflow types that skip the Schwartz diagnosis), reach for a proven skeleton from `.claude/skills/write-strike/references/micro-formulas.md` (PAS, BAB, AIDA, PASTOR, FAB, 4Ps); FAB doubles as the feature-to-benefit antidote to feature-dumping. For long-form and persuasion-heavy work, Schwartz and the gut-check above lead; the formulas are the quick-draft tool, not the strategy layer.
...
2. **Every voice in bolter-tones**: read the Available Voices table in `.claude/skills/bolter-tones/SKILL.md` and offer each person listed, using the table's **Menu blurb** as the option description.
...
- Follow the nine-section structure in `.claude/skills/bolt-content-formats/references/customer-story-template.md`.
...
Read `.claude/skills/write-strike/references/web-copy-best-practices.md` before drafting any website copy. It contains the 5-question test, above-the-fold framework, headline formula, CTA specificity rules, objection handling, mobile-first writing, and page-type structure templates (homepage, feature, pricing, solution, persona, industry, and single-use landing page). GEO/AEO rules from `.claude/skills/write-strike/references/SEO-GEO-drafting.md` also apply to any page that will live on bolt.new.
...
3. **Schwartz version**: diagnose the reader's awareness level and market sophistication using `.claude/skills/write-strike/references/schwartz-5x5-matrix.md`, then write the ad to match that intersection. The matrix determines whether to lead with the problem, the mechanism, the proof, or the offer.
...
- Follow the asset structure in `.claude/skills/bolt-content-formats/references/sales-enablement-template.md` (battle card, objection handler, or competitive one-sheet).
...
- Follow the BOM in `.claude/skills/bolt-content-formats/references/webinar-bom-template.md`. Work through each section the user needs; don't dump the whole template at once.
...
After drafting, run the `noslops` skill against the draft (`.claude/skills/noslops/SKILL.md`), the single source of truth for the AI-tell filter and the 35/50 scoring rubric. Score the draft on directness, rhythm, trust, authenticity, and density. 35/50 minimum to pass. Fix every violation before presenting. If the draft scores below 35, revise and re-audit.
...
- **Schwartz version**: Diagnose the reader's awareness level and market sophistication using `.claude/skills/write-strike/references/schwartz-5x5-matrix.md`, then rewrite the headline, lead, and copy structure to match that intersection. Particularly useful when the draft feels right in substance but the opening doesn't land.
...
Run a focused SEO/GEO research pass to ground the ideation in data, then weave findings into the conversation (not a report dump). The tool choreography, the exact triggers, and what's connected all live in `bolt-seo-geo`, mapped in `.claude/skills/bolt-seo-geo/references/seo-geo-toolkit.md`. Delegate there rather than duplicating it here.
...
Reference `.claude/skills/write-strike/references/schwartz-5x5-matrix.md` for the full matrix. If an idea sits at an unusual intersection (e.g., Unaware + Stage 5), flag it; those combinations require specific approaches and longer copy.
...
- **Flesch Reading Ease:** Check the target persona's readability targets in bolt-buyer-personas (`.claude/skills/bolt-buyer-personas/SKILL.md`). Each persona has a specific Flesch range and jargon tolerance.
...
SEO/GEO data, tooling, and the full stage-by-stage orchestration (which `/seo-*` skill to run at research, draft, and post-draft, plus what data is connected) all live in the **`bolt-seo-geo`** skill, mapped in `.claude/skills/bolt-seo-geo/references/seo-geo-toolkit.md`. This skill does not duplicate that choreography; it delegates to it. (Drafting itself is governed by `.claude/skills/write-strike/references/SEO-GEO-drafting.md`, which is a writing rule, not a tool.)
