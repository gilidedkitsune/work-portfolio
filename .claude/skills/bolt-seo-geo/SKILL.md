...
The bundled puller is `.claude/skills/bolt-seo-geo/scripts/seo_pull.py` (on a standard install: `.claude/skills/bolt-seo-geo/scripts/seo_pull.py`). It defaults to bolt.new, so the data commands need no arguments. Run it with the system `python3`.

```bash
# GA4 — traffic by channel, last 28 days (default)
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py ga4
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py ga4 --days 90

# Search Console — top queries, last 28 days (default)
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py gsc
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py gsc --dimension page --limit 50 --days 90

# List the GA4 properties this account can see (to find or confirm an ID)
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py properties

# One-time (or when a pull fails with an auth error): sign in / refresh
python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py auth
```
...
7. **Sign in:** `python3 .claude/skills/bolt-seo-geo/scripts/seo_pull.py auth` — a browser opens; approve the read-only scopes. This writes `~/.config/claude-seo/token.json`. Done.
...
- **The bundle has no secrets.** Only `SKILL.md`, `.claude/skills/bolt-seo-geo/scripts/seo_pull.py`, and `.claude/skills/bolt-seo-geo/references/`. The puller reads credentials from `~/.config/claude-seo/`, which is **outside** the skill — so packaging or copying the skill never carries a password.
